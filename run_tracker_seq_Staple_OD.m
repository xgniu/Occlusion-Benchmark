function run_tracker_seq_Staple_OD(seq_name)
    addpath('.\Staple');
    addpath('.\KCF');
...
%     update_visualization = show_video(seq_name, img_files);
    ...
    for frame = 1:numel(img_files)
        ...
        if frame == 1
            ...
            target.tracker = Staple(im,center,target_sz);
        else
            track_output = target.tracker.track(im); %(x,y,width,height)     
...
%     precision_plot(seq_name,bboxes,ground_truth);
    fps= frame/time;
    save(['.\Results1\' seq_name '_Staple_OD'],'bboxes');   
